<template>
	<div class="main-content" :style='{"minHeight":"100vh","padding":"20px 20px 20px","overflow":"hidden","color":"#000","background":"#e9ecf3","fontSize":"16px","position":"relative"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid #ddd","padding":"20px 20px 0","margin":"0px 0 0","overflow":"hidden","borderRadius":"0px","flexWrap":"wrap","background":"#fff","display":"flex","width":"auto","fontSize":"inherit","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"padding":"0px","margin":"0 0 10px","color":"#fff","flexWrap":"wrap","textAlign":"left","flexDirection":"row","background":"none","display":"flex","width":"auto","fontSize":"14px"}'>
					<el-button class="add" v-if="isAuth('kebiaoxinix','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 0px","fontSize":"inherit","color":"inherit","display":"inline-block"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('kebiaoxinix','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu15" :style='{"margin":"0 0px","fontSize":"inherit","color":"inherit","display":"inline-block"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('kebiaoxinix','课程表')" type="success" @click="timeableClick()">
						<span class="icon iconfont icon-shuju17" :style='{"margin":"0 0px","fontSize":"inherit","color":"inherit","display":"inline-block"}'></span>
						课程表
					</el-button>
			...
			<div :style='{"border":"0px solid #bababa","padding":"0","margin":"0 0 0 0","borderRadius":"2px","background":"rgba(255,255,255,1)","width":"calc(100% - 0px)","fontSize":"14px","float":"right"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="edit" v-if=" isAuth('kebiaoxinix','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('kebiaoxinix','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<el-dialog :visible.sync="timeableVisible" width="60%" @close="timeableClose" title="课程表">
			<timeable :events="timeableList" :time="timeList" :afternoonLength="afternoonLength" :length="allLength" :morningLength="morningLength"></timeable>
		</el-dialog>
...
import AddOrUpdate from "./add-or-update";
	import timeable from '@/components/common/timeable'
	export default {
		data() {
			return {
				searchForm: {
		...
		methods: {
			...
			timeableClose(){
				this.afternoonLength = 0
				this.morningLength = 0
				this.timeableList = []
				this.$forceUpdate()
			},
			async timeableClick() {
				await this.$http({
					url: 'scheduletime/list',
					method: 'get',
					params: {
						page: 1,
						limit: 100
					...
					if (res.data && res.data.code == 0) {
						for(let x in res.data.data.list){
							res.data.data.list[x].sjd = this.dateState('2020-01-01' + ' ' + res.data.data.list[x].endtime + ':00')
							if(res.data.data.list[x].sjd=='上午'){
								this.morningLength++
							}
							if(res.data.data.list[x].sjd=='下午'){
								this.afternoonLength++
							}
						}
					
						this.allLength = res.data.data.list.length
				...
				for (let x in this.dataList) {
					let indexList = this.dataList[x].sectionnum.split(',')
					let name =
						this.dataList[x].riqi
						+ '<br>' + this.dataList[x].changdi
					arr.push({
						week: Number(this.dataList[x].week),
						title: this.dataList[x].kechengmingcheng,
						content: name,
						start: Number(indexList[0]),
						end: indexList.length > 1 ? Number(indexList[indexList.length - 1]) : Number(indexList[0]),
					})
				}
				this.timeableList = arr
				this.timeableVisible = true
			},
			contentStyleChange() {
		...
		getDataList() {
			...
			if(this.searchForm.sectionnum&&this.searchForm.sectionnum.length){
				let sectionnumArr = []
				for(let i in this.searchForm.sectionnum){
					sectionnumArr.push('%' + this.searchForm.sectionnum[i] + '%')
				}
				params['sectionnum'] = sectionnumArr.join(',')
			}
			...
			if(user.xingming){
				params['xingming'] = user.xingming
			}
			this.$http({
				url: "kebiaoxinix/page",
				method: "get",
				params: params
			}).then(({ data }) => {
				if (data && data.code === 0) {
		...
		kebiaoxinixstatusChange(e,row){
			if(row.status==0){
				row.passwordwrongnum = 0
			}
			this.$http({
				url:'kebiaoxinix/update',
				method:'post',
				data:row
			}).then(res=>{
				if(row.status==1){
